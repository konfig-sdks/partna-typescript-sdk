...
import { readableStreamToString, PartnaError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PartnaError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PartnaError(e, e.response?.data)
                }
                if (innerError instanceof PartnaError) {
                    // Got "PartnaError" from the above try block
                    throw innerError;
                }
